...
install() {
    bash <(curl -Ls ${groxy}https://raw.githubusercontent.com/Night-stars-1/x-ui/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
install_bbr() {
    ...
    bash <(curl -L -s ${groxy}https://raw.githubusercontent.com/teddysun/across/master/bbr.sh)
    echo ""
    before_show_menu
}

update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate ${groxy}https://github.com/Night-stars-1/x-ui/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
